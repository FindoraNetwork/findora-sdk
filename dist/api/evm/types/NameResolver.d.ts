...
export type ContractContext = Web3ContractContext<NameResolver, NameResolverMethodNames, NameResolverEventsContext, NameResolverEvents>;
...
export interface NameResolverEventsContext {
    DelManager(parameters: {
        filter?: {
            owner?: string | string[];
            delOperator?: string | string[];
        };
        fromBlock?: number;
...
export type NameResolverMethodNames = 'addManager' | 'delManager' | 'delName' | 'initialize' | 'isManager' | 'name' | 'owner' | 'renounceOwnership' | 'setName' | 'transferOwnership';
