...
export const prepareAnonTransferOperationBuilder = async (
  ...
  const ledger = await getLedger();
  const xfrPublicKeyReceiver = await ledger.public_key_from_base64(xfrPublicKeyReceiverString);
...
const processAbarToAbarCommitmentResponse = async (
  ...
  for (const commitmentKey of commitmentKeys) {
    ...
    const [commitmentXfrPublicKey, commitmentNumericAssetType, commitmentAmountInWei] = commitmentEntity;
...
export const barToAbarAmount = async (
  ...
  const barToAbarResult = await barToAbar(walletInfo, [utxoToUse.sid], receiverXfrPublicKey);
...
export const barToAbar = async (
  ...
  try {
    xfrPublicKey = await ledger.public_key_from_base64(receiverXfrPublicKey);
  } catch (error) {
...
export const getNullifierHashesFromCommitments = async (
  ...
  for (const givenCommitment of givenCommitmentsList) {
    ...
    try {
      ...
      console.log(
        `getOwnedAbars for '${anonKeys.publickey}'->'${givenCommitment}' returned an error. ${
          (error as Error).message
        }`,
        console.log('Full Error', error),
      );
...
export const getAbarBalance = async (unspentAbars: FindoraWallet.OwnedAbarItem[], anonKeys: WalletKeypar) => {
  ...
  const balanceInfo: AnonWalletBalanceInfo = {
    xfrPublicKey: anonKeys.publickey,
    balances,
  };
...
export const genNullifierHash = async (
  ...
  try {
    const hash = ledger.gen_nullifier_hash(myOwnedAbar, abarOwnerMemo, xfrKeyPair, myMTLeafInfo);
...
export const decryptAbarMemo = async (
  ...
  const xfrKeyPair = anonKeys.keypair;
  ...
  try {
    decryptedAbar = ledger.try_decrypt_axfr_memo(abarOwnerMemo, xfrKeyPair);
  } catch (error) {
    return false;
  }
  const result = {
